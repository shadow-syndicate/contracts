...
def test_sale_happy_path(accounts, GenesisMintDebug, roach_nft):
    ...
    with reverts("Account limit reached"):
        genesis_sale.mintStage1noSig(1, 0, PRICE, {'from':buyer, 'amount': 100})
    ...
    with reverts("Insufficient money"):
        genesis_sale.mintStage1noSig(1, ALLOWED, PRICE, {'from':buyer, 'amount': 99})

    genesis_sale.mintStage1noSig(1, ALLOWED, PRICE, {'from':buyer, 'amount': 100})
    ...
    tx = genesis_sale.mintStage1noSig(4, ALLOWED, PRICE, {'from':buyer, 'amount': 400})
...
def test_withdraw(accounts, GenesisMintDebug, roach_nft):
    ...
    genesis_sale.mintStage1noSig(13, 13, 0, {'from':buyer, 'amount': 1500, 'gasPrice': 0})
...
def test_sale_ended_soldout(accounts, GenesisMintDebug, roach_nft):
    ...
    genesis_sale.mintStage1noSig(3, 10, 0, {'from':buyer, 'amount': 300})
...
def test_buy_left_tokens(accounts, GenesisMintDebug, roach_nft):
    ...
    genesis_sale.mintStage1noSig(5, 5, 0, {'from':buyer, 'amount': 500, 'gas_price': 0})

    status = genesis_sale.getSaleStatus(buyer, 0)
    assert status[0] == 2, "stage2 ended"

    with reverts("Sale not active"):
        genesis_sale.mintStage1noSig(1, 1, 0, {'from':buyer, 'amount': 100})
...
def test_not_enough_money(accounts, GenesisMintDebug, roach_nft):
    ...
    with reverts("Insufficient money"):
        genesis_sale.mintStage1noSig(5, 5, PRICE, {'from':buyer, 'amount': 200, 'gas_price': 0})
...
def test_buy_all_tokens_on_sale(accounts, GenesisMintDebug, roach_nft):
    ...
    genesis_sale.mintStage1noSig(3, 10, PRICE, {'from':buyer, 'amount': 300})
...
def test_total_supply(accounts, GenesisMintDebug, roach_nft):
    ...
    genesis_sale.mintOperator(buyer, 1)
...
def test_operator_mint_count_limit(accounts, GenesisMintDebug, roach_nft):
    ...
    genesis_sale.mintStage1noSig(1, 1, 100, {'from':buyer, 'amount': 100, 'gas_price': 0})

    assert genesis_sale.totalMinted() == 1, "totalMinted"

    genesis_sale.mintOperator(buyer, 2, {'from':accounts[0]})
    assert genesis_sale.totalMinted() == 3, "totalMinted"
    assert roach_nft.balanceOf(buyer) == 3

    with reverts("Sale is over"):
        genesis_sale.mintOperator(buyer, 1,{'from':accounts[0]})
...
#         genesis_sale.mint(1, sig["limit"], sig["price"],
#                             sig["sig"]["v"], sig["sig"]["r"], sig["sig"]["s"],
#                             {'from':buyer, 'amount': 100})
#     isValid = genesis_sale.isValidSignature(sig["account"], sig["limit"], sig["price"],
#                                             sig["sig"]["v"], sig["sig"]["r"], sig["sig"]["s"])
...
def test_whitelist_price(accounts, GenesisMintDebug, roach_nft):
    ...
    with reverts("Insufficient money"):
        genesis_sale.mintStage1noSig(1, 10, PRICE * 2, {'from':buyer, 'amount': PRICE})

    WL_PRICE = 10
    before = buyer.balance()
    genesis_sale.mintStage1noSig(1, 10, WL_PRICE, {'from':buyer, 'amount': PRICE, 'gasPrice': 0})
    after = buyer.balance()
    assert before - after == PRICE, "take all money"

    WL_PRICE = 0
    before = buyer.balance()
    genesis_sale.mintStage1noSig(1, 10, WL_PRICE, {'from':buyer, 'gasPrice': 0})
    after = buyer.balance()
