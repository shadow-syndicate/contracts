...
///      TokenSeed is unpredictable in mint stage for all parties:
...
///      So TokenSeed is unpredictable, but fixed and can be checked in any time after reveal.
///      Genome is generated using tokenSeed. For same tokenSeed there will be equal genomes.
///      After genesis sale is finished game will publish secretSeed and anyone will be able to
///      check tokenSeeds and corresponding genomes.
///      Genome is signed by secret private key and transferred from Polygon network to Ethereum
///      as part of reveal process.
/// @dev GenomeProviderPolygon is used onl for testing because of mocked VRF request.
